...
local function OpenGarageMenu(data)
    QBCore.Functions.TriggerCallback('alpha-garages:server:GetGarageVehicles', function(result)
...
local function DepositVehicle(veh, data)
    ...
    QBCore.Functions.TriggerCallback('alpha-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            ...
            TriggerServerEvent('alpha-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('alpha-garages:server:UpdateOutsideVehicle', plate, nil) end
...
local function CreateGaragePed(garage, index)
    -- Get the ped model based on garage type or specific override
    local pedModel = garage.pedModel
    if not pedModel then
        if Config.PedModels[garage.type] then
            pedModel = Config.PedModels[garage.type]
        else
            pedModel = Config.PedModels['default']
        end
    end
    ...
    -- Create the ped
    local ped = CreatePed(4, GetHashKey(pedModel), garage.takeVehicle.x, garage.takeVehicle.y, garage.takeVehicle.z - 1.0, garage.takeVehicle.w, false, true)
    ...
    -- Add to table for cleanup
    garagePeds[index] = ped
    ...
    exports['qb-target']:AddTargetEntity(ped, {
        options = {
            {
                type = "client",
                event = "alpha-garages:client:OpenGarageTarget",
                icon = "fas fa-car",
                label = "Access Garage",
                garageData = {
                    indexgarage = index,
                ...
                canInteract = function()
                    return GetVehiclePedIsIn(PlayerPedId(), false) == 0 -- Only when not in a vehicle
                end
            },
            {
                type = "client",
                event = "alpha-garages:client:ParkVehicleTarget",
                icon = "fas fa-parking",
                label = "Park Vehicle",
                garageData = {
                    indexgarage = index,
                ...
                canInteract = function()
                    ...
                    if vehicle == 0 then return false end -- Not in a vehicle
                    if garage.type == 'depot' then return false end -- Can't park at depot
                    return IsVehicleAllowed(garage.category, vehicle) -- Check if vehicle class is allowed
                end
            }
        },
        distance = 2.5,
...
local function CreateZone(index, garage, zoneType)
    -- Create the zone using vector3 coordinates from the vector4
    local zoneCoords = vector3(garage.takeVehicle.x, garage.takeVehicle.y, garage.takeVehicle.z)
    local zone = CircleZone:Create(zoneCoords, 10.0, {
...
local function CreateBlipsZones()
    ...
    for index, garage in pairs(Config.Garages) do
        ...
        -- Create garage attendant ped
        CreateGaragePed(garage, index)
        ...
        if isPointInside then
            ...
            CreateThread(function()
                while listenForKey do
                    Wait(0)
                    -- Check if player is in a vehicle to enable parking functionality
                    if IsControlJustReleased(0, 38) and GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
            ...
            -- Only show parking text when in a vehicle
            if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                local displayText = Lang:t('info.park_e')
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('alpha-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('alpha-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('alpha-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('alpha-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('alpha-garages:client:OpenGarageTarget', function(data)
    if data.garageData then
        OpenGarageMenu(data.garageData)
    end
end)

RegisterNetEvent('alpha-garages:client:ParkVehicleTarget', function(data)
    if data.garageData then
        local currentVehicle = GetVehiclePedIsIn(PlayerPedId(), false)
        if currentVehicle ~= 0 then
            DepositVehicle(currentVehicle, data.garageData)
        end
    end
end)

RegisterNetEvent('alpha-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('alpha-garages:client:takeOutGarage', function(data)
    QBCore.Functions.TriggerCallback('alpha-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            QBCore.Functions.TriggerCallback('alpha-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                ...
                TriggerServerEvent('alpha-garages:server:updateVehicleState', 0, vehPlate)
...
RegisterNetEvent('alpha-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        QBCore.Functions.TriggerCallback('alpha-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('alpha-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('alpha-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('alpha-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
...
-- Get vehicle history
RegisterNUICallback('getVehicleHistory', function(data, cb)
    local plate = data.plate
    
    QBCore.Functions.TriggerCallback('alpha-garages:server:GetVehicleHistory', function(history)
        SendNUIMessage({
...
RegisterNUICallback('repairVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('alpha-garages:server:RepairVehicle', function(result)
        if result.success then
            QBCore.Functions.Notify('Vehicle repaired for $' .. result.cost, 'success')
        else
            QBCore.Functions.Notify(result.reason or 'Failed to repair vehicle', 'error')
        end
        cb(result)
    end, plate)
end)
...
AddEventHandler('onResourceStop', function(resource)
    ...
    for _, v in pairs(houseGarageZones) do
        v:destroy()
    end
    
    -- Clean up all garage peds
    for _, ped in pairs(garagePeds) do
        if DoesEntityExist(ped) then
            DeleteEntity(ped)
        end
